...
func forkAndExecInChild1(argv0 *byte, argv, envv []*byte, chroot, dir *byte, attr *ProcAttr, sys *SysProcAttr, pipe int) (r1 uintptr, err1 syscall.Errno, p [2]int, locked bool) {
    ...
    if sys.Unshareflags != 0 {
        ...
        if sys.Unshareflags&syscall.CLONE_NEWUSER != 0 && sys.GidMappings != nil {
            dirfd := int(_AT_FDCWD)
            if fd1, _, err1 = syscall.RawSyscall6(syscall.SYS_OPENAT, uintptr(dirfd), uintptr(unsafe.Pointer(&psetgroups[0])), uintptr(syscall.O_WRONLY), 0, 0, 0); err1 != 0 {
                goto childerror
            }
            r1, _, err1 = syscall.RawSyscall(syscall.SYS_WRITE, uintptr(fd1), uintptr(unsafe.Pointer(&setgroups[0])), uintptr(len(setgroups)))
            if err1 != 0 {
                goto childerror
            }
            if _, _, err1 = syscall.RawSyscall(syscall.SYS_CLOSE, uintptr(fd1), 0, 0); err1 != 0 {
                goto childerror
            }

            if fd1, _, err1 = syscall.RawSyscall6(syscall.SYS_OPENAT, uintptr(dirfd), uintptr(unsafe.Pointer(&pgid[0])), uintptr(syscall.O_WRONLY), 0, 0, 0); err1 != 0 {
                goto childerror
            }
            r1, _, err1 = syscall.RawSyscall(syscall.SYS_WRITE, uintptr(fd1), uintptr(unsafe.Pointer(&gidmap[0])), uintptr(len(gidmap)))
            if err1 != 0 {
                goto childerror
            }
            if _, _, err1 = syscall.RawSyscall(syscall.SYS_CLOSE, uintptr(fd1), 0, 0); err1 != 0 {
        ...
        if sys.Unshareflags&syscall.CLONE_NEWUSER != 0 && sys.UidMappings != nil {
            dirfd := int(_AT_FDCWD)
            if fd1, _, err1 = syscall.RawSyscall6(syscall.SYS_OPENAT, uintptr(dirfd), uintptr(unsafe.Pointer(&puid[0])), uintptr(syscall.O_WRONLY), 0, 0, 0); err1 != 0 {
                goto childerror
            }
            r1, _, err1 = syscall.RawSyscall(syscall.SYS_WRITE, uintptr(fd1), uintptr(unsafe.Pointer(&uidmap[0])), uintptr(len(uidmap)))
            if err1 != 0 {
                goto childerror
            }
            if _, _, err1 = syscall.RawSyscall(syscall.SYS_CLOSE, uintptr(fd1), 0, 0); err1 != 0 {
    ...
    if len(sys.AmbientCaps) != 0 {
        ...
        if _, _, err1 := syscall.RawSyscall(syscall.SYS_CAPGET, uintptr(unsafe.Pointer(&caps.hdr)), uintptr(unsafe.Pointer(&caps.data[0])), 0); err1 != 0 {
            goto childerror
        }
    ...
    if pipe < nextfd {
        ...
        if err1 == syscall.ENOSYS {
            ...
            syscall.RawSyscall(fcntl64Syscall, uintptr(nextfd),syscall. F_SETFD, syscall.FD_CLOEXEC)
        } else if err1 != 0 {
            goto childerror
        }
        pipe = nextfd
    ...
    for i = 0; i < len(fd); i++ {
        ...
        if fd[i] == int(i) {
            ...
            _, _, err1 = syscall.RawSyscall(fcntl64Syscall, uintptr(fd[i]), syscall.F_SETFD, 0)
            if err1 != 0 {
                goto childerror
            }
            continue
